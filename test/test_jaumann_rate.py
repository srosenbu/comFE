...
def jaumann_rotate_3d_python(L,sigma,del_t):
    ...
    return np.array([sigma_out[0,0],sigma_out[1,1],sigma_out[2,2],2**0.5*sigma_out[1,2],2**0.5*sigma_out[0,2],2**0.5*sigma_out[0,1]]) 
...
def test_jaumann_rotation():
    ...
    sigma_python = np.array(
        [
            jaumann_rotate_3d_python(L_i.reshape(3,3),sigma_i,del_t) for L_i, sigma_i in zip(L.reshape(-1,9), sigma.reshape(-1,6))
        ]
    ).flatten()

...
use crate::rub::RUBConstParameters;

use core::panic;
use std::collections::HashMap;

#[derive(Debug)]
pub struct GradientRUB3D {
    parameters: RUBConstParameters,
}

impl ConstitutiveModel for GradientRUB3D {
    ...
    fn evaluate_ip(&mut self, ip: usize, del_t: f64, input: &QValueInput, output: &mut QValueOutput) {
        ...
        let lambda_nonlocal = input.get_scalar(Q::EqNonlocalPlasticStrain, ip);
        let damage_0 = input.get_scalar(Q::Damage, ip);
        let damage_1 = (1. - ((self.parameters.alpha_0 - lambda_nonlocal) / self.parameters.e_f).exp())
            .max(damage_0);
        ...
        let yield_surface =  {
            if damage_0 == 0.0 {
                ...
                initial_yield_surface * (1. - damage_1) * hardening_factor + damage_1 * residual_surface
            }
        };

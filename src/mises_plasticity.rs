...
impl ConstitutiveModel<6, 2, 7, 4, 4> for MisesPlasticity3D {
    ...
    fn evaluate(
        _time: f64,
        _del_time: f64,
        _strain: SVectorView<f64, 6>,
        del_strain: SVectorView<f64, 6>,
        stress: &mut SVectorViewMut<f64, 6>,
        tangent: Option<&mut SMatrixViewMut<f64, 6,6>>,
        history: &mut Self::History,
        parameters: &Self::Parameters,
    ) {
        let mu = parameters.mu;
        let kappa = parameters.kappa;
        let y_0 = parameters.y_0;
        let h = parameters.h;

        let alpha = history.alpha;

        

        let (p_0, s_0) = stress.vol_dev();
        let (eps_trace, eps_dev) = del_strain.trace_dev();
        let p_1 = p_0 + kappa * eps_trace;
        ...
        if s_tr_eq < sigma_y {
            // Elastic step
            stress.copy_from(&(p_1 * SYM_ID_6 + s_tr));
            if let Some(tangent) = tangent {
                tangent.copy_from(&(kappa * SYM_ID_6_OUTER_SYM_ID_6 + 2. * mu * PROJECTION_DEV_6));
            }
            return;
        } else {
            let del_alpha = (s_tr_eq - sigma_y) / (3. * mu + h);
            ...
            // determine the plastic strain
            let n = s_tr / s_tr_eq;
            history.plastic_strain+= del_gamma * n;
            history.alpha += del_alpha;

            stress.copy_from(&(p_1 * SYM_ID_6 + theta * s_tr));
